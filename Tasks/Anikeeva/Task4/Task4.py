while True:
    task = (input('Please enter the maximum number of characters per line. More than 35 symbols:\n'))
    if not task.isdigit():   
...
with open('text.txt', 'r', encoding = 'utf-8') as start:    
    with open('example_40_text.txt', 'w', encoding = 'utf-8') as finish:   
        ...
        for i in text:           
            if task - len(''.join(list)) >= len(i):  
                ...
                list = [i, ' ']
        finish.write(''.join(list))
print ('Here is your version of the file')